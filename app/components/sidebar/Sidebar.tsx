...
const Sidebar = ({ onFilterChange, selectedCategory, onAddNote }: SidebarProps) => {
  const categories = [
  ...
  return (
    <div className="flex flex-col space-y-2 p-4 text-lg text-gray-600">
      ...
      <div className="flex flex-col min-w-44">
        {categories.map((category) => (
          <button
            ...
            className={`text-left font-medium hover:bg-gray_secondary flex flex-row items-center space-x-2 ${selectedCategory === category.id ? 'bg-gray-200' : ''}`}
          >
      ...
      <button onClick={handleAddNoteClick} className="mt-4 p-2 bg-blue-500 font-semibold text-white rounded">
        Nueva Nota
      </button>
    </div>
  );
